...
| D-019 | 2026-09-24 | Static security headers now; nonce-based Content-Security-Policy added in A4 via `proxy.ts` | Nonce CSP needs per-request rendering, which signed-in pages get with auth; avoids forcing marketing pages dynamic | Accepted |
| D-020 | 2026-09-24 | `APP_PREVIEW=synthetic` lets developers view signed-in screens before auth exists; env validation refuses it in production; removed in A4 | Lets UI work proceed without weakening production protection | Accepted (temporary) |
