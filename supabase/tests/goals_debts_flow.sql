...
begin
  ...
  if (select name from public.goals where id = g1) = 'Emergency fund' then passed := passed + 1; else failures := failures || text 'name not trimmed'; end if;
  -- A second goal doesn't change the plan the user already has
  g2 := public.create_goal('goal', 'Holiday', 800000, 0, 0, null);
  select planned_cents into n from public.budget_lines where budget_id = bid and category_id = cat;
  if n = 80000 then passed := passed + 1; else failures := failures || text 'second goal changed plan'; end if;
  -- A sinking fund needs a due month
  begin
    perform public.create_goal('sinking_fund', 'School fees', 720000, 60000, 0, null);
    failures := failures || text 'fund without due month';
  ...
  if exists (select 1 from public.categories where system_key = 'sinking_funds') then passed := passed + 1; else failures := failures || text 'no sinking category'; end if;
  ...
  if n = 1 then passed := passed + 1; else failures := failures || text 'add money transaction'; end if;
  if exists (select 1 from public.audit_events where action = 'goal.money_added' and entity_id = c) then passed := passed + 1; else failures := failures || text 'no audit event'; end if;
  -- Taking out can't make saved negative (saved is 6 400 + 500 = 6 900)
  begin
    perform public.goal_move_money(g1, 'out', 690001, current_date);
    failures := failures || text 'took out more than saved';
  ...
  if n = 1 then passed := passed + 1; else failures := failures || text 'take out is not a refund'; end if;
  ...
  begin
    ...
    failures := failures || text 'linked transaction edited';
  ...
  begin
    ...
    failures := failures || text 'linked transaction soft-deleted';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  ...
  if r = 'archived' and exists (select 1 from public.transactions_manual where id = tid) then passed := passed + 1; else failures := failures || text 'archive goal'; end if;
  begin
    perform public.goal_move_money(g1, 'in', 100, current_date);
    failures := failures || text 'money added to archived goal';
  ...
  if r = 'deleted' then passed := passed + 1; else failures := failures || text 'empty goal archived'; end if;
  ...
  select planned_cents into n from public.budget_lines where budget_id = bid and category_id = cat;
  if n = 45000 and (select note from public.debts where id = d1) is null then passed := passed + 1; else failures := failures || text 'debt line or note'; end if;
  ...
  begin
    ...
    failures := failures || text 'overpayment without confirm';
  exception when invalid_parameter_value then
  ...
  if bal = 0 and (select paid_off_on from public.debts where id = d1) = current_date then passed := passed + 1; else failures := failures || text 'paid off'; end if;
  begin
    perform public.debt_record_payment(d1, 100, current_date, null, true);
    failures := failures || text 'payment on paid-off debt';
  exception when invalid_parameter_value then passed := passed + 1;
  end;
  select count(*) into n from public.transactions_manual where category_id = cat;
  ...
  then passed := passed + 1; else failures := failures || text 'set balance'; end if;
  if exists (select 1 from public.audit_events where action = 'debt.balance_updated') then passed := passed + 1; else failures := failures || text 'no balance audit'; end if;
  begin
    perform public.debt_set_balance(d1, -1, current_date, null);
    failures := failures || text 'negative balance';
  exception when invalid_parameter_value then passed := passed + 1;
  end;
  ...
  begin
    ...
    failures := failures || text 'unknown checklist key';
  exception when check_violation then passed := passed + 1;
  end;
  ...
  begin
    ...
    failures := failures || text 'B added money to A''s fund';
  exception when invalid_parameter_value then passed := passed + 1;
  end;
  begin
    perform public.debt_record_payment(d1, 100, current_date, null, true);
    failures := failures || text 'B paid A''s debt';
  exception when invalid_parameter_value then passed := passed + 1;
  end;
  begin
    perform public.remove_debt(d1, 'delete');
    failures := failures || text 'B removed A''s debt';
  exception when invalid_parameter_value then passed := passed + 1;
  end;
  ...
  select count(*) into n from public.transactions_manual where category_id = cat;
  if r = 'deleted' and n = 0 then passed := passed + 1; else failures := failures || text 'delete debt'; end if;
  execute 'reset role';
