...
begin
  ...
  then passed := passed + 1; else failures := failures || 'first save'; end if;
  if (select checklist #>> '{done,checkin}' from public.budgets where id = bid) = 'true' then passed := passed + 1; else failures := failures || 'checklist not ticked'; end if;
  ...
  then passed := passed + 1; else failures := failures || 'edit'; end if;
  -- A month without a budget can still be checked in
  perform public.save_checkin('2026-07', null, null, null);
  if exists (select 1 from public.monthly_checkins where period = '2026-07') then passed := passed + 1; else failures := failures || 'no-budget month'; end if;
  ...
  if exists (select 1 from public.audit_events where action = 'export.created' and entity_id = e) then passed := passed + 1; else failures := failures || 'no export audit'; end if;
  begin
    update public.exports set expires_at = now() + interval '1 year' where id = e;
    failures := failures || 'export updated';
  ...
  begin
    ...
    failures := failures || 'pdf export without period';
  exception when check_violation then passed := passed + 1;
  end;
  ...
  if n = 0 then passed := passed + 1; else failures := failures || 'B reads A''s check-ins'; end if;
  select count(*) into n from public.exports;
  if n = 0 then passed := passed + 1; else failures := failures || 'B reads A''s exports'; end if;
  begin
    insert into public.exports (household_id, kind, period, spec_version) values (ha, 'pdf_summary', '2026-08', '1.0');
    failures := failures || 'B created an export for A';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  perform public.save_checkin('2026-08', 'B''s own', null, '[]');
  select count(*) into n from public.monthly_checkins;
  if n = 1 then passed := passed + 1; else failures := failures || 'B check-in'; end if;
  execute 'reset role';
  if (select went_well from public.monthly_checkins where household_id = ha and period = '2026-08') = 'Edited'
  then passed := passed + 1; else failures := failures || 'B changed A''s check-in'; end if;
