...
begin
  ...
  if (select price_cents from public.plan_offer()) = 1000 and (select open from public.plan_offer()) then passed := passed + 1; else failures := failures || text 'offer'; end if;
  select * into pay from public.start_checkout();
  if pay.amount_cents = 1000 and (select status from public.payments where id = pay.payment_id) = 'pending' then passed := passed + 1; else failures := failures || text 'checkout'; end if;
  ...
  begin
    ...
    failures := failures || text 'user inserted a payment';
  ...
  begin
    ...
    failures := failures || text 'user changed a payment';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  begin
    insert into public.entitlements (household_id, starts_at, ends_at) values (ha, now(), now() + interval '1 year');
    failures := failures || text 'user granted access';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  begin
    perform 1 from private.plans;
    failures := failures || text 'user read plans';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  ...
  begin
    ...
    failures := failures || text 'signed-in user applied a notification';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  ...
  begin
    ...
    failures := failures || text 'wrong secret accepted';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  execute 'reset role';
  ...
  execute 'reset role';
  if r = 'rejected' and (select reject_reason from public.payments where id = pay3.payment_id) = 'merchant' then passed := passed + 1; else failures := failures || text 'merchant'; end if;
  ...
  execute 'reset role';
  if r = 'pending' and not exists (select 1 from public.entitlements where household_id = ha) then passed := passed + 1; else failures := failures || text 'pending'; end if;
  execute 'set local role anon';
  ...
  if exists (select 1 from public.audit_events where action = 'entitlement.granted' and entity_id = pay.payment_id) then passed := passed + 1; else failures := failures || text 'no audit'; end if;

  -- Duplicate and late notifications change nothing
  execute 'set local role anon';
  r := public.payfast_apply_itn(secret, pay.payment_id, 'pf-1', 'COMPLETE', 1000, true);
  execute 'reset role';
  if r = 'duplicate' and (select count(*) from public.entitlements where household_id = ha) = 1 then passed := passed + 1; else failures := failures || text 'duplicate'; end if;
  execute 'set local role anon';
  r := public.payfast_apply_itn(secret, pay.payment_id, 'pf-1', 'FAILED', 1000, true);
  execute 'reset role';
  if r = 'duplicate' and (select status from public.payments where id = pay.payment_id) = 'complete' then passed := passed + 1; else failures := failures || text 'late failure'; end if;
  ...
  execute 'reset role';
  if r = 'ignored' and (select count(*) from public.entitlements where household_id = ha) = 1 then passed := passed + 1; else failures := failures || text 'reused pf id'; end if;
  -- Unknown payment id
  execute 'set local role anon';
  if public.payfast_apply_itn(secret, gen_random_uuid(), 'pf-x', 'COMPLETE', 1000, true) = 'unknown' then passed := passed + 1; else failures := failures || text 'unknown'; end if;
  -- anon can't read payments
  begin
    perform 1 from public.payments;
    failures := failures || text 'anon read payments';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  execute 'reset role';
  perform set_config('request.jwt.claims', json_build_object('sub', a, 'role', 'authenticated')::text, true);
  ...
  begin
    ...
    failures := failures || text 'checkout while entitled';
  ...
  if n = 0 and (select count(*) from public.entitlements) = 0 then passed := passed + 1; else failures := failures || text 'B sees A''s payments'; end if;
  execute 'reset role';
  ...
  begin
    perform public.start_checkout();
    failures := failures || text 'anon checkout';
  exception when insufficient_privilege then passed := passed + 1;
  end;
  execute 'reset role';
  ...
  begin
    perform public.start_checkout();
    failures := failures || text 'closed plan checkout';
  exception when invalid_parameter_value then passed := passed + 1;
  end;
  execute 'reset role';
  ...
  then passed := passed + 1; else failures := failures || text 'deletion left payments'; end if;
